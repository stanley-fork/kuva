...
`--no-sort` preserves the input row order instead of sorting descending by value. `--no-threshold` hides the reference line. `--no-legend` hides the legend (shown by default).
